...
-----------------------------------*/
  ...
  var activeSticky = $("#sticky-header"),
    $winDow = $($window);
  function updateBodyPaddingForHeader() {
    if (!activeSticky.length) return;
    var h = activeSticky.outerHeight();
    if (activeSticky.hasClass('is-sticky')) {
  ...
  if (activeSticky.length) {
    if ($($window).scrollTop() > 0) activeSticky.addClass('is-sticky');
    updateBodyPaddingForHeader();
